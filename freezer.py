...
class FreezerCreator:
    ...
    def __init__(self, cli, shelves_per_freezer: int, racks_per_shelf:int, trays_per_rack:int, boxes_per_tray:int):
    ...
    def create_freezer(self, name: str):
        ...
        ## shelves
        shelves = self.create_tier(self.shelves_per_freezer, "shelf", self.racks_per_shelf,1 )
        ...
        self.add_to_parent_tier(shelves, self.shelves_per_freezer, self.racks_per_shelf, racks) 
        ...
        return {"freezer": [root], "shelves": shelves, "racks": racks, "trays":trays, "boxes": boxes}
        
    
    def create_tier(self, n, name_prefix, rows, columns):
        items = []
        for i in range(n):
            item = self.cli.create_grid_container(f"{name_prefix}-{i}", rows,columns, can_store_samples=False)
            items.append(item['globalId'])
        return items
        
    def add_to_parent_tier(self, parents, parents_per_gp, items_per_parent, items):
        print (f"adding {items} to {parents} where ppg = {parents_per_gp} and ipp={items_per_parent}")
...
trays_per_rack=2
boxes_per_tray=2
freezerFactory = FreezerCreator(cli, shelves_per_freezer, racks_per_shelf, trays_per_rack, boxes_per_tray)
